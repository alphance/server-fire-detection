...
import threading
import time
import datetime # Added for human-readable time
import io
import collections
...
# DHT_PIN_3 = getattr(board, "D17", None) if board else None # Check your wiring!
# DHT_PIN_4 = getattr(board, "D27", None) if board else None # Check your wiring!
...
# ---------------------------
def setup_sensors():
    sensors = [None, None, None, None] # dht1, dht2, dht3, dht4
    mlx = None
    
    # Helper to init a specific pin
    def init_dht(pin, name):
        if adafruit_dht and pin:
            try:
                # use_pulseio=False is safer for Pi 5/Linux
    ...
    while True:
        ...
        if mlx:
            try:
                ...
                # FIX: Filter out startup "ghost" noise
                ...
                with data_lock:
                    ...
                    # FIX: Use readable time string instead of unix timestamp
...
def update_dashboard(n, alert_source, view_opts, dht_temp_lim, dht_hum_lim, thermal_lim, thermal_mode, email_addr):
    ...
    if valid_email:
        if alert_source == 'dht' and dht_temp_lim is not None and dht_hum_lim is not None:
            ...
            for i in range(1, 5):
                t_val = dht[f't{i}']
                h_val = dht[f'h{i}']
                if t_val is not None and t_val > dht_temp_lim: triggers.append(f"S{i} Temp: {t_val:.1f}C")
                if h_val is not None and h_val > dht_hum_lim: triggers.append(f"S{i} Hum: {h_val:.1f}%")
    ...
    history_fig.update_layout(title='Thermal Trends (Time)')

    # DHT Bar Chart (4 Sensors)
    ...
    for i in range(1, 5):
        t = dht[f't{i}']
        s_text = f"Sensor {i}: {t:.1f}°C" if t is not None else f"Sensor {i}: No Data"
