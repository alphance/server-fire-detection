...
DEFAULT_DHT_HUM_MIN_THRESHOLD = 30  # Min humidity % (Static danger)
DEFAULT_THERMAL_TEMP_THRESHOLD = 50 # Surface hotspot limit °C

MLX_WIDTH = 32
MLX_HEIGHT = 24
MAX_HISTORY = 100 # Keep 100 points of history for graphs
...
# Helper to create deque structure
def create_history():
    return {
        ...
        if (current_time - last_dht_read_time) > DHT_POLL_INTERVAL:
            ...
            with data_lock:
                ...
                if results[0][0] is not None:
                    latest_data["dht_history"][1]["time"].append(time_str)
                    latest_data["dht_history"][1]["temp"].append(results[0][0])
                    latest_data["dht_history"][1]["hum"].append(results[0][1])
                
                # Update Sensor 2
                latest_data["dht"]["t2"], latest_data["dht"]["h2"] = results[1]
                if results[1][0] is not None:
                    latest_data["dht_history"][2]["time"].append(time_str)
                    latest_data["dht_history"][2]["temp"].append(results[1][0])
                    latest_data["dht_history"][2]["hum"].append(results[1][1])

                # Update Sensor 3
                latest_data["dht"]["t3"], latest_data["dht"]["h3"] = results[2]
                if results[2][0] is not None:
                    latest_data["dht_history"][3]["time"].append(time_str)
                    latest_data["dht_history"][3]["temp"].append(results[2][0])
                    latest_data["dht_history"][3]["hum"].append(results[2][1])

                # Update Sensor 4
                latest_data["dht"]["t4"], latest_data["dht"]["h4"] = results[3]
                if results[3][0] is not None:
                    latest_data["dht_history"][4]["time"].append(time_str)
                    latest_data["dht_history"][4]["temp"].append(results[3][0])
                    latest_data["dht_history"][4]["hum"].append(results[3][1])
...
# NEW: Helper to generate line graph for specific sensor
def generate_dht_history_image(sensor_idx):
    buf = io.BytesIO()
    try:
        with data_lock:
            # Copy data safely
        ...
        plt.title(f'History: Sensor {sensor_idx}')
        plt.legend()
        plt.grid(True)
        # Simplify x-axis to not crowd it
        if len(times) > 5:
...
def send_alert_email_thread(target_email, subject, body, frame, bad_sensor_idx=None):
    def runner():
        ...
        try:
            ...
            # Attach DHT Graph (if a specific sensor failed)
            if bad_sensor_idx:
                dht_img_data = generate_dht_history_image(bad_sensor_idx)
                if dht_img_data:
                    msg.attach(MIMEImage(dht_img_data, name=f'sensor_{bad_sensor_idx}_history.png'))
...
def update_dashboard(n, alert_source, view_opts, dht_temp_lim, dht_hum_min, dht_hum_max, thermal_lim, thermal_mode, email_addr):
    ...
    # Flags for email logic
    is_dht_temp_alert = False
    is_dht_hum_alert = False
    is_thermal_alert = False
    
    # Track WHICH sensor failed for graph attachment
    bad_sensor_idx = None 
    ...
    if valid_email:
        ...
        # 3. Construct Nuanced Subject Line
        if triggers:
            ...
            if (current_time - last_alert_time) > ALERT_COOLDOWN:
                ...
                # Pass 'bad_sensor_idx' so the thread knows which graph to attach
                send_alert_email_thread(email_addr, subject, body, frame, bad_sensor_idx)
    ...
    for i in range(1, 5):
        ...
        current_t = dht[f't{i}']
        title_str = f"S{i}: {current_t:.1f}°C" if current_t else f"Sensor {i}"
    ...
    for i in range(1, 5):
        t = dht[f't{i}']
        h = dht[f'h{i}']
        s_text = f"S{i}: {t:.1f}°C / {h:.1f}% | " if t is not None else f"S{i}: -- | "
