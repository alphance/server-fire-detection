...
Features: 
- Distinct Subject Lines (Airflow vs Hotspot vs Humidity)
- 4-Sensor Array (DHTs)
...
# --- DEFAULT THRESHOLDS ---
DEFAULT_DHT_TEMP_THRESHOLD = 45   # Exhaust air is usually hotter (35-45°C is normal for exhaust)
DEFAULT_DHT_HUM_THRESHOLD = 60    # Max humidity in %
DEFAULT_THERMAL_TEMP_THRESHOLD = 50 # Surface temp (Plastic starts softening > 60°C)
...
def update_dashboard(n, alert_source, view_opts, dht_temp_lim, dht_hum_lim, thermal_lim, thermal_mode, email_addr):
    ...
    # Flags to determine Email Subject Nuance
        ...
        if alert_source == 'dht' and dht_temp_lim is not None and dht_hum_lim is not None:
            for i in range(1, 5):
                ...
                if h_val is not None and h_val > dht_hum_lim: 
                    triggers.append(f"S{i} Humidity: {h_val:.1f}%")
