...
def send_alert_email_thread(target_email, subject, body, frame, bad_sensor_idx=None, bad_sensor_name="Sensor"):
    def runner():
        ...
        try:
            ...
            if bad_sensor_idx:
                dht_img_data = generate_dht_history_image(bad_sensor_idx, bad_sensor_name)
                if dht_img_data:
                    msg.attach(MIMEImage(dht_img_data, name=f'{bad_sensor_name.replace(" ","_")}_history.png'))
    ...
    if valid_email:
        ...
        if triggers:
            ...
            if (current_time - last_alert_time) > ALERT_COOLDOWN:
                ...
                bad_sensor_name = sensor_names[bad_sensor_idx] if bad_sensor_idx else "Sensor"
                send_alert_email_thread(email_addr, subject, body, frame, bad_sensor_idx, bad_sensor_name)
